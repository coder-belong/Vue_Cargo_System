    ...
    <v-dialog v-model="createTaskmDialogVisible" :persistent="false" max-width="600px">
      <v-card>
        ...
        <v-form ref="form" v-model="valid" lazy-validation class="task_form">
          ...
          <v-select v-model="detail.association" :items="selects" label="相关" required></v-select>
          <v-textarea name="desc" label="描述信息" v-model="detail.desc"></v-textarea>
        </v-form>
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="primary" text @click="createTaskmDialogVisible = false">关闭</v-btn>
          <v-btn color="primary" @click="createTaskmDialogVisible = false">保存</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
