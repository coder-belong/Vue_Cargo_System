    ...
    <div class="login_container" :class="{ login_mlmlh: mlmlh }">
        ...
        <v-toolbar absolute flat dark color="transparent" min-width="100%">
            ...
            <v-btn text style="margin: 0 20px">
                <v-icon style="margin-right: 10px">mdi-fingerprint</v-icon>注册
            </v-btn>
...
export default {
    ...
    methods: {
        // 注册按钮的点击
        async handleSignBtn() {
            let { username, password, confirmPwd } = this.form
            // 1. 前台校验表单数据
            ...
            // 2. 发送网络请求登录
            let { meta, token } = await register(username, password)
            if (meta.status !== 200) {
                return this.$message({
                    showClose: true,
                    message: meta.msg,
